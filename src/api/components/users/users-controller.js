...
async function getUsers(request, response, next) {
  try {
    // Extract pagination, sorting, and filtering options from request query
    const { page_number, page_size, sort, search } = request.query;

    // Call getUsers function in userService with options
    ...
    // Return paginated users as response
...
async function createPurchase(request, response, next) {
  try {
    ...
    const success = await usersService.createPurchase(
      product,
      description,
      price,
      quantity
    ...
    return response
      .status(200)
      .json({ message: 'The purchase is successful!' });
  } catch (error) {
    return next(error);
  }
}
...
async function getPurchases(request, response, next) {
  try {
    const purchases = await usersService.getPurchases();
...
async function getPurchase(request, response, next) {
  try {
    const purchase = await usersService.getPurchase(request.params.id);
...
async function updatePurchase(request, response, next) {
  try {
    ...
    const success = await usersService.updatePurchase(id, price, quantity);
    if (!success) {
      throw errorResponder(
        errorTypes.UNPROCESSABLE_ENTITY,
